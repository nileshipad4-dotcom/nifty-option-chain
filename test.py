...
if len(csv_files) < 3:
    st.error("Need at least 3 CSV files in the 'data' folder.")
    st.stop()
...
# Default indices logic to avoid errors if list is short
idx1 = 0 if len(filtered_ts) > 0 else 0
idx2 = 1 if len(filtered_ts) > 1 else 0
idx3 = 2 if len(filtered_ts) > 2 else 0

t1 = c1.selectbox("TS1 (Current)", filtered_ts, index=idx1)
t2 = c2.selectbox("TS2 (Previous)", filtered_ts, index=idx2)
t3 = c3.selectbox("TS3 (Oldest)", filtered_ts, index=idx3)
...
for i, d in enumerate([df1, df2, df3]):
    ...
    if not all(col in d.columns for col in req_cols):
        continue
        
    dfs.append(
        d[req_cols].rename(columns={
            "Stock_LTP": f"ltp_{i}",
...
# ==================================================
# Note: Iterating rows is slow in Pandas, but kept to maintain original logic
for stk, g in df.groupby("Stock"):
    g = g.sort_values("Strike").reset_index()
    
    # Calculate Sums per Strike
    ...
    low_atm = max(0, atm_idx - 2)
    high_atm = min(len(g) - 1, atm_idx + 2)
    
    orig_indices = g.loc[low_atm:high_atm, "index"]
    window_vals = df.loc[orig_indices, "diff"].fillna(0)
    atm_avg = window_vals.mean()
    
    # Assign ATM Avg to all rows for this stock (for filtering later)
...
table = df[[
    ...
    "Stock": "stk", "Strike": "str", "ltp_0": "ltp"
})
...
def filter_near_ltp(df, n=5):
    ...
    return pd.concat(blocks, ignore_index=True) if blocks else pd.DataFrame(columns=df.columns)
...
col_pe, col_ce, col_n = st.columns([1, 1, 1])
...
col_pe.toggle("PE Min", key="pe_min", on_change=pe_callback)
col_ce.toggle("CE Min", key="ce_min", on_change=ce_callback)
BOTTOM_N = col_n.number_input("Bottom N", min_value=5, max_value=100, value=20, step=5)

PE_MIN = st.session_state.pe_min
CE_MIN = st.session_state.ce_min

# ==================================================
# 📌 IDENTIFY BOTTOM STOCKS (FOR HIGHLIGHTING)
# ==================================================
stock_sums = (
    display_df.groupby("stk")
    .agg(pe_sum=("pe_x", "sum"), ce_sum=("ce_x", "sum"))
    .reset_index()
)

bottom_pe_stocks = set(stock_sums.sort_values("pe_sum", ascending=True).head(BOTTOM_N)["stk"])
bottom_ce_stocks = set(stock_sums.sort_values("ce_sum", ascending=True).head(BOTTOM_N)["stk"])

# ==================================================
# 🧹 SORT DISPLAY DATAFRAME
# ==================================================
if PE_MIN or CE_MIN:
    if PE_MIN:
        stock_sort = stock_sums.sort_values("pe_sum", ascending=True)
    elif CE_MIN:
        stock_sort = stock_sums.sort_values("ce_sum", ascending=True)
    
    ordered_stocks = stock_sort["stk"].tolist()
    sorted_blocks = []
    for stk in ordered_stocks:
        block = display_df[display_df["stk"] == stk].sort_values("str")
...
def atm_blue(data):
    ...
    if "stk" not in data.columns: return styles
...
# ==================================================
# 📈 STOCK PRESENCE IN TOP DIFF STRIKES
# ==================================================
st.subheader("📈 Stock Presence in Top Diff Strikes")
cA, cB, cC = st.columns(3)
TOP_N = cA.number_input("Top N Strikes", min_value=10, max_value=500, value=150, step=10)
MIN_COUNT = cB.number_input("Min Count", min_value=1, max_value=20, value=6, step=1)
...
top_n_df = sorted_df.merge(display_df[["stk", "str"]], on=["stk", "str"], how="inner").head(TOP_N)

stock_summary = (
    top_n_df.groupby("stk")
    .agg(count=("diff", "size"), total_ch=("total_ch", "first"), ch=("ch", "first"))
    .reset_index()
)
stock_summary = stock_summary[stock_summary["count"] >= MIN_COUNT].sort_values("count", ascending=False)
...
top_n_df_23 = sorted_df_23.merge(display_df[["stk", "str"]], on=["stk", "str"], how="inner").head(TOP_N)

stock_summary_23 = (
    top_n_df_23.groupby("stk")
    .agg(count=("diff_23", "size"))
    .reset_index()
)
stock_summary_23 = stock_summary_23[stock_summary_23["count"] >= MIN_COUNT].sort_values("count", ascending=False)

# Common stocks between both analyses
...
fmt2 = {"count": "{:.0f}", "total_ch": "{:.2f}", "ch": "{:.2f}"}
...
# ==================================================
# 📋 DETAILED VIEW (STOCKS FROM 'BASED ON DIFF')
# ==================================================
...
stock_a = sc1.selectbox("Select Stock A", stock_list, index=0)
stock_b = sc2.selectbox("Select Stock B", stock_list, index=0)
...
if stock_a or stock_b:
    ...
    with dsc1: show_stock_table(stock_a)
    with dsc2: show_stock_table(stock_b)
...
if not uptrend_blocks:
    ...
    rank_df = pd.DataFrame(up_ranking, columns=["stk", "score"]).sort_values("score", ascending=False)
    final_blocks = []
    for stk in rank_df["stk"]:
        # Find the block in list corresponding to this stock
        for block in uptrend_blocks:
            if block["stk"].iloc[0] == stk:
...
if not downtrend_blocks:
    ...
    # Sort blocks by score
    rank_df = pd.DataFrame(down_ranking, columns=["stk", "score"]).sort_values("score", ascending=False)
    final_blocks = []
    for stk in rank_df["stk"]:
        for block in downtrend_blocks:
            if block["stk"].iloc[0] == stk:
                final_blocks.append(block)
                break
